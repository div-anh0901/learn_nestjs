...
import { DataStudents, FecthDataCourse } from '../../../utils/type-request';
...
const CourseTable: React.FC<Props> = ({ data, meta,setMeta, onCreate,setFilter,filter,resetPage }) => {
  const [isModalOpen, setModalOpen] = useState(false);
  const { isOpen, openModal, closeModal } = useModal();
  const [sortKey, setSortKey] = useState<keyof FecthDataCourse>('id');
  ...
  const toggleSort = (key: keyof FecthDataCourse) => {
    if (sortKey === key) {
  ...
  return (
    <div className="p-4 border rounded-xl bg-white dark:bg-white/[0.02] overflow-auto">
      ...
      <div className="flex items-center">
          <h2 className="text-xl font-semibold">Courses ({meta.total})</h2>
          <input
            type="text"
            placeholder="Search..."
            value={filter}
      ...
      <table className="min-w-[800px] w-full text-left border border-gray-200 dark:border-white/[0.05]">
        <thead className="bg-gray-100 dark:bg-white/[0.05]">
          <tr>
            ...
            {[ 'name', 'title', 'time', 'major',"actions"].map((key) => (
              <th
                key={key}
                className="px-4 py-3 cursor-pointer text-gray-600 dark:text-gray-300"
                onClick={() => toggleSort(key as keyof FecthDataCourse)}
              >
        ...
        <tbody>
          {sortedData.map(row => (
            <tr key={row.id} className="border-t border-gray-100 dark:border-white/[0.05]">
