...
import { DataStudents, DataTeacher } from '../../../utils/type-request';
...
import { deleteStudentById, deleteTeacherById, findStudentById, findTeacherById, updateStudentById, updateTeacherById } from '../../../utils/api-axios';
...
const TeacherTable: React.FC<Props> = ({ data, meta,setMeta, onCreate,setFilter,filter,resetPage }) => {
  const [isModalOpen, setModalOpen] = useState(false);
  const { isOpen, openModal, closeModal } = useModal();
  const [sortKey, setSortKey] = useState<keyof DataTeacher>('_id');
  ...
  const toggleSort = (key: keyof DataTeacher) => {
    if (sortKey === key) {
  ...
  const fecthTeacherById = useCallback((id: string, callback: ()=>{})=>{
    findTeacherById(id).then(data =>{
            setForm({
              ...form,
              email: data.email,
      ...
      validateForm(form, 
        async () => {
            try{   // Close only on success
              await updateTeacherById(idEdit, form);
              resetPage()
              ToastSuccess('Edit teacher success!')
             
          } catch (err) {
              ToastError('Edit teacher failed')
          }
          closeModal()
        }
      )
  ...
  const confirmDelete = ()=>{
        deleteTeacherById(idDeleted).then(data =>{
        ToastSuccess('Delete teacher success!');
        resetPage();
        setConfirmOpen(false);
    }).catch(err => 
    {
      ToastError('Delete teacher failed');
        setConfirmOpen(false);
    }
    )
